      ...
      <AnimatePresence mode="wait">
       {!hasConfirmedAge ? (
            ...
            <AnimatePresence mode="wait">
              {!isHonest ? (
                ...
                <motion.div key="honest-msg" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full">
                  ...
                  <h2 className="text-lg font-bold tracking-[0.2em] mb-4 text-black dark:text-white italic text-center">期待下次相遇</h2>
                  <p className="text-xs leading-relaxed opacity-60 text-black dark:text-slate-400 text-center">喵<br/>喵喵喵</p>
                  <button onClick={() => setIsHonest(false)} className="mt-8 text-[10px] underline opacity-40 uppercase font-bold text-black dark:text-white block mx-auto">Return</button>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
          ...
          <motion.div key="main-content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col min-h-screen">
              ...
              {showUpdateNotice && !currentStory && (
                <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-6 bg-black/30 dark:bg-black/60 backdrop-blur-sm">
                  <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-[440px] flex flex-col items-center">
                    <div className={`bg-[#F5F5F5] dark:bg-[#1a1a1a] border ${isDarkMode ? 'border-white/10' : 'border-black/10'} p-10 rounded-2xl shadow-2xl w-full text-center`}>
                      <h4 className="text-[10px] uppercase tracking-[0.4em] font-sans font-black opacity-30 mb-10 text-black dark:text-white">Latest Updates</h4>
                      <div className="space-y-8">
                        {latestUpdates.map(story => {
                          const latestChapter = story.chapters && story.chapters.length > 0 ? story.chapters[story.chapters.length - 1].title : null;
                          return (
                            <div key={story.id}>
                              ...
                              <p className="text-[9px] uppercase tracking-[0.2em] opacity-40 dark:opacity-50 mt-4 font-sans font-black text-black dark:text-slate-400">Archived: {story.author}</p>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    {/* 灰色极简圆钮 */}
                    <button onClick={() => setShowUpdateNotice(false)} className="mt-8 w-12 h-12 rounded-full bg-slate-500/20 hover:bg-slate-500/40 text-slate-600 dark:text-slate-200 flex items-center justify-center transition-all shadow-lg border border-black/5 dark:border-white/10"><X size={24} /></button>
                  </motion.div>
                </div>
              )}
            </AnimatePresence>
              ...
              <AnimatePresence mode="wait">
                {!currentStory ? (
                    ...
                    <section className="max-w-[700px] mx-auto">
                      {currentItems.map(s => (
                        <motion.button key={s.id} whileHover={{ x: 5 }} onClick={() => handleStoryClick(s)} className={`w-full grid grid-cols-[1fr_auto] py-8 border-b transition-colors text-left ${isDarkMode ? 'border-white/10 hover:border-white/30 text-white' : 'border-black/5 hover:border-black/20 text-[#333]'}`}>
                          ...
                          <div className="col-span-full flex gap-4 text-[10px] opacity-40 uppercase tracking-widest font-sans font-black"><span>{s.author}</span><span>{s.date?.replace(/-/g, '.')}</span>{s.chapters && <span>{s.chapters.length} 章节</span>}</div>
                        </motion.button>
                      ))}
                    </section>
                    {totalPages > 1 && (
                  ...
                  <motion.div key="chapters" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[600px] mx-auto py-12">
                    <div className="mb-12 text-center text-center"><h2 className="text-2xl font-black font-serif italic text-black dark:text-white">{currentStory.title}</h2><p className="text-xs opacity-40 tracking-widest uppercase font-sans font-black text-black dark:text-slate-400">Directory / 目录</p></div>
                  ...
                  <motion.div key="content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[700px] mx-auto">
                    <header className="mb-16 border-b border-black/5 dark:border-white/10 pb-12 text-center font-sans">
                        ...
                        {(() => {
                          ...
                          return parts.map((part, idx) => {
                            ...
                            if (/\[\s*bubble:/.test(part)) {
                              ...
                              return (<div key={idx} className={`flex ${isRight ? 'justify-end' : 'justify-start'} my-4`}><div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[14px] shadow-sm tracking-tight ${isRight ? 'bg-[#607d8b] text-white rounded-tr-none' : (isDarkMode ? 'bg-slate-800 text-slate-100 rounded-tl-none' : 'bg-slate-200 text-slate-800 rounded-tl-none')}`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-2" />)}</div></div>);
                            }
                            if (/\[\s*bvid:/.test(part)) {
                            ...
                            if (/\[\s*ytid:/.test(part)) {
                                ...
                                return (<div key={idx} className="my-10 aspect-video w-full overflow-hidden rounded-2xl shadow-2xl bg-black ring-1 ring-white/10"><iframe src={`https://www.youtube.com/embed/${ytid}?rel=0&autoplay=0`} className="w-full h-full border-none" allowFullScreen loading="lazy" /></div>);
                            }
            ...
            <footer className="py-20 px-6 border-t border-black/5 dark:border-white/10 text-center opacity-40 text-[10px] tracking-widest font-serif uppercase text-black dark:text-white">
