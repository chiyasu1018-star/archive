...
fetch(`https://raw.githubusercontent.com/chiyasu1018-star/archive/main/public/stories/index.json?v=${Date.now()}`)
  ...
  {(() => {
    ...
    // 2. 将全文拆分为“块”和“纯文本”的数组
    const parts = cleanRaw.split(blockRegex);

    return parts.map((part, idx) => {
      if (!part || part.trim() === '') return null; // 过滤掉由于拆分产生的空字符串
      ...
      if (/\[\s*quote\s*\]/.test(part)) {
        ...
        return (
          <blockquote key={idx} className={`my-10 pl-5 border-l-4 italic py-6 rounded-r-xl ${isDarkMode ? 'border-slate-600 bg-white/5 text-slate-400' : 'border-slate-300 bg-slate-100/30 text-slate-500'}`}>
            {inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}
          </blockquote>
        );
      }

      // --- B. 方框块渲染 ---
      else if (/\[\s*box\s*\]/.test(part)) {
        const inner = part.replace(/\[\s*\/?box\s*\]/g, '').trim();
        return (
          <div key={idx} className={`my-10 p-8 border rounded-2xl text-sm leading-relaxed shadow-sm ${isDarkMode ? 'bg-zinc-900 border-white/10 text-slate-300' : 'bg-slate-100/60 border-slate-200 text-slate-700'}`}>
            {inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}
          </div>
        );
      }
      ...
      // --- D. BiliBili 视频渲染 ---
        ...
        return (
          <div key={idx} className="my-10 aspect-video w-full overflow-hidden rounded-2xl shadow-2xl bg-black ring-1 ring-white/10">
            <iframe src={`//player.bilibili.com/player.html?bvid=${bvid}&page=1&high_quality=1&danmaku=0&autoplay=0`} className="w-full h-full border-none" allowFullScreen loading="lazy" />
          </div>
        );
      }

      // --- E. YouTube 视频渲染 (精准位置修复) ---
      else if (/\[\s*youtube:/.test(part)) {
      ...
      // --- F. 分割线渲染 ---
        ...
        return <hr key={idx} className={`my-16 border-t ${isDarkMode ? 'border-white/10' : 'border-black/10'}`} />;
      }

      // --- G. 纯文本渲染 (Fallback) ---
      else {
        return part.split('\n').map((line, lIdx) => {
          if (line.trim() === '') return <div key={lIdx} className="h-6" />; // 稍微缩小空行高度
