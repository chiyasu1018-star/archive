...
import { 
  ChevronLeft, ChevronUp, Moon, Sun, ShieldAlert, Heart, BookOpen, X 
} from 'lucide-react';
            ...
            <main className="pt-24 pb-20 px-6 max-w-4xl mx-auto flex-grow w-full">
              <AnimatePresence mode="wait">
                {!currentStory ? (
                    ...
                    <section className="max-w-[700px] mx-auto">
                      {currentItems.map(s => (
                        <motion.button key={s.id} whileHover={{ x: 5 }} onClick={() => handleStoryClick(s)} className={`w-full grid grid-cols-[1fr_auto] py-8 border-b transition-colors text-left ${isDarkMode ? 'border-white/10 hover:border-white/30 text-white' : 'border-black/5 hover:border-black/20 text-[#333]'}`}>
                          ...
                          <div className="col-span-full flex gap-4 text-[10px] opacity-40 dark:opacity-60 uppercase tracking-widest font-sans font-black text-black dark:text-white"><span>{s.author}</span><span>{s.date?.replace(/-/g, '.')}</span>{s.chapters && <span>{s.chapters.length} 章节</span>}</div>
                        </motion.button>
                      ))}
                    </section>
                    {totalPages > 1 && (
                  ...
                  <motion.div key="content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[700px] mx-auto text-center text-center">
                    ...
                    {reading ? (<div className="py-20 text-center opacity-20 tracking-widest text-xs uppercase animate-pulse text-black dark:text-white">Loading Content...</div>) : (
                      <article style={{ fontSize: `${fontSize}px`, lineHeight: '1.9' }} className={`mb-24 font-serif text-black dark:text-white/90 text-justify text-center`}>
                        {(() => {
                          ...
                          return parts.map((part, idx) => {
                            ...
                            if (/\[\s*box\s*\]/.test(part)) {
                              ...
                              return (<div key={idx} className={`my-10 p-8 border rounded-2xl text-sm leading-relaxed shadow-sm ${isDarkMode ? 'bg-zinc-900 border-white/10 text-slate-300' : 'bg-slate-100/60 border-slate-200 text-slate-700'} text-center`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}</div>);
                            }
                            if (/\[\s*bubble:/.test(part)) {
                              const isRight = part.includes(':R');
                              const inner = part.replace(/\[\s*bubble:[LR]\s*\]/g, '').replace(/\[\s*\/bubble\s*\]/g, '').trim();
                              return (<div key={idx} className={`flex ${isRight ? 'justify-end' : 'justify-start'} my-4 text-center`}><div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[14px] shadow-sm tracking-tight ${isRight ? 'bg-[#607d8b] text-white rounded-tr-none' : (isDarkMode ? 'bg-slate-800 text-slate-100 rounded-tl-none' : 'bg-slate-200 text-slate-800 rounded-tl-none')} text-center`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-2" />)}</div></div>);
                            }
                            if (/\[\s*bvid:/.test(part)) {
                              const bvid = part.match(/bvid:\s*([a-zA-Z0-9]+)/)?.[1];
                              return (<div key={idx} className="my-10 aspect-video w-full overflow-hidden rounded-2xl shadow-2xl bg-black ring-1 ring-white/10 text-center"><iframe src={`//player.bilibili.com/player.html?bvid=${bvid}&page=1&high_quality=1&danmaku=0&autoplay=0`} className="w-full h-full border-none" allowFullScreen loading="lazy" /></div>);
                            }
                            if (/\[\s*ytid:/.test(part)) {
                                const ytid = part.match(/ytid:\s*([a-zA-Z0-9_-]+)/)?.[1];
                                return (<div key={idx} className="my-10 aspect-video w-full overflow-hidden rounded-2xl shadow-2xl bg-black ring-1 ring-white/10 text-center"><iframe src={`https://www.youtube.com/embed/${ytid}?rel=0&autoplay=0`} className="w-full h-full border-none" allowFullScreen loading="lazy" /></div>);
                            }
                            if (trimmedPart === '---') return <hr key={idx} className={`my-16 border-t ${isDarkMode ? 'border-white/10' : 'border-black/10'} text-center`} />;
                            return part.split('\n').map((line, lIdx) => {
                              if (line === '') return <div key={lIdx} className="h-8 text-center" />;
                              return <p key={`${idx}-${lIdx}`} className="mb-5 min-h-[1.5em] text-center" dangerouslySetInnerHTML={{ __html: applyInlineStyles(line) }} />;
                            });
                          });
                        })()}
                      </article>
                    )}
                    <div className={`flex flex-col sm:flex-row items-center justify-between gap-6 py-12 border-t border-dashed ${isDarkMode ? 'border-white/20' : 'border-black/10'} text-center`}>
                       <p className={`text-sm font-black tracking-widest ${isDarkMode ? 'text-white/60' : 'text-black/40'} text-center`}>如果喜欢这篇文章，请务必去支持一下原作者。</p>
                       <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className={`flex items-center gap-2 px-6 py-3 rounded-full text-[10px] font-black tracking-[0.2em] uppercase transition-all border ${isDarkMode ? 'border-white/20 hover:bg-white hover:text-black text-white' : 'border-black/10 hover:border-black hover:text-white text-black'} text-center`}>Top / 回到顶部 <ChevronUp size={14} /></button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            </main>
            <footer className="py-20 px-6 border-t border-black/5 dark:border-white/10 text-center opacity-40 text-[10px] tracking-widest font-serif uppercase text-black dark:text-white text-center">
              <div className="max-w-[600px] mx-auto space-y-3 normal-case leading-relaxed mb-12 text-center font-black text-center"><p>本站仅作为 Postype 平台 녘랜 (花汪) 同人文作品的翻译交流与存档使用，版权归原作者所有。</p><p>站内内容全是机翻，如有侵权请联系删除。</p><p className="font-bold text-center">联系微博：<span>@恋花症-</span></p></div>
              <p onClick={(e) => { if (e.detail === 5) setIsAdmin(true); }} className="italic font-sans tracking-[0.2em] cursor-default select-none text-center text-center">© 2026 HW ARCHIVE.</p>
            </footer>
          </motion.div>
        )}
      </AnimatePresence>
