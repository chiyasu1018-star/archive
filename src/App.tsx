              ...
              <AnimatePresence mode="wait">
                {!currentStory ? (
                  <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <header className="text-center py-12"><h1 className="text-3xl font-black tracking-[0.2em] mb-4 text-black dark:text-white text-center">花汪档案馆</h1></header>
                    <section className="max-w-[700px] mx-auto">
                      {currentItems.map(s => (
                        <motion.button key={s.id} whileHover={{ x: 5 }} onClick={() => handleStoryClick(s)} className={`w-full grid grid-cols-[1fr_auto] py-8 border-b transition-colors text-left ${isDarkMode ? 'border-white/10 hover:border-white/30 text-white' : 'border-black/5 hover:border-black/20 text-[#333]'}`}>
                          ...
                          <div className="col-span-full flex gap-4 text-[10px] opacity-40 uppercase tracking-widest font-sans font-black text-center text-center"><span>{s.author}</span><span>{s.date?.replace(/-/g, '.')}</span>{s.chapters && <span>{s.chapters.length} 章节</span>}</div>
                        </motion.button>
                      ))}
                    </section>
                    {totalPages > 1 && (
                      <div className="flex justify-center items-center gap-12 mt-20 py-10 border-t border-dashed border-black/5 dark:border-white/10 text-center text-center">
                  ...
                  <motion.div key="chapters" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[600px] mx-auto py-12 text-center text-center">
                    ...
                    <div className="grid gap-4">
                      {currentStory.chapters?.map((chapter, idx) => (
                        <button key={idx} onClick={() => loadFullStory(currentStory, chapter.fileName, chapter.title)} className={`p-6 border rounded-2xl text-left transition-all group flex justify-between items-center ${isDarkMode ? 'border-white/10 bg-white/5 hover:bg-white/10' : 'border-black/5 bg-white hover:bg-black/5'} text-center text-center`}>
                          <div><span className="text-[10px] opacity-30 block mb-1 font-sans font-black uppercase tracking-widest text-black dark:text-slate-400 text-center text-center text-center">Chapter {idx + 1}</span><span className="text-lg group-hover:pl-2 transition-all duration-300 font-serif font-black italic text-black dark:text-white text-center text-center">{chapter.title}</span></div>
                          <div className="text-[10px] opacity-30 font-sans tracking-widest uppercase text-right font-black text-black dark:text-slate-400 text-center text-center">{chapter.autoWordCount ? `${chapter.autoWordCount.toLocaleString()} 字` : '...'}</div>
                        </button>
                      ))}
                    </div>
                  </motion.div>
                ) : (
                  <motion.div key="content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[700px] mx-auto text-center text-center text-center">
                    <header className="mb-16 border-b border-black/5 dark:border-white/10 pb-12 font-sans text-center text-center text-center text-center">
                      <h2 className="text-4xl font-serif font-black italic mb-8 leading-tight text-black dark:text-white text-center text-center">{currentStory.title}{currentStory.currentChapterTitle && (<span className="block text-xl opacity-60 mt-4 font-serif font-medium text-center text-center text-center">— {currentStory.currentChapterTitle}</span>)}</h2>
                      <div className="text-[11px] uppercase tracking-[0.2em] opacity-50 dark:opacity-70 space-y-1 font-black text-black dark:text-slate-300 text-center text-center text-center text-center"><p>作者: {currentStory.author}</p><p>时间: {currentStory.date?.replace(/-/g, '.')}</p><p>字数: {reading ? '...' : (currentStory.wordCount?.toLocaleString() || '...')}</p></div>
                      <a href={currentStory.sourceLink} target="_blank" rel="noopener noreferrer" className={`inline-block mt-8 text-[13px] font-black tracking-[0.2em] underline underline-offset-8 decoration-1 transition-opacity ${isDarkMode ? 'text-blue-400 hover:text-blue-300' : 'text-[#607d8b] hover:text-[#455a64]'} text-center text-center text-center text-center`}>原链接 SOURCE →</a>
                    </header>
                    {reading ? (<div className="py-20 text-center opacity-20 tracking-widest text-xs uppercase animate-pulse text-black dark:text-white text-center text-center">Loading Content...</div>) : (
                      <article style={{ fontSize: `${fontSize}px`, lineHeight: '1.9' }} className={`mb-24 font-serif text-black dark:text-white/90 text-justify`}>
                        {(() => {
                          const raw = currentStory.content || '';
                          const cleanRaw = raw.replace(/\r\n/g, '\n');
                          
                          // 🌟 重点修改：正则中加入了对 [ytid:...] 的识别
                          ...
                          return parts.map((part, idx) => {
                            ...
                            if (/\[\s*quote\s*\]/.test(part)) {
                              ...
                              return (<blockquote key={idx} className={`my-10 pl-5 border-l-4 italic py-6 rounded-r-xl ${isDarkMode ? 'border-slate-600 bg-white/5 text-slate-400' : 'border-slate-300 bg-slate-100/30 text-slate-500'} text-center`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}</blockquote>);
                            }
                            if (/\[\s*box\s*\]/.test(part)) {
                              const inner = part.replace(/\[\s*\/?box\s*\]/g, '').trim();
                              return (<div key={idx} className={`my-10 p-8 border rounded-2xl text-sm leading-relaxed shadow-sm ${isDarkMode ? 'bg-zinc-900 border-white/10 text-slate-300' : 'bg-slate-100/60 border-slate-200 text-slate-700'} text-center text-center`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}</div>);
                            }
                            if (/\[\s*bubble:/.test(part)) {
                              const isRight = part.includes(':R');
                              const inner = part.replace(/\[\s*bubble:[LR]\s*\]/g, '').replace(/\[\s*\/bubble\s*\]/g, '').trim();
                              return (<div key={idx} className={`flex ${isRight ? 'justify-end' : 'justify-start'} my-4 text-center text-center`}><div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[14px] shadow-sm tracking-tight ${isRight ? 'bg-[#607d8b] text-white rounded-tr-none' : (isDarkMode ? 'bg-slate-800 text-slate-100 rounded-tl-none' : 'bg-slate-200 text-slate-800 rounded-tl-none')} text-center text-center`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-2" />)}</div></div>);
                            }
                            // Bilibili
                            if (/\[\s*bvid:/.test(part)) {
                              const bvid = part.match(/bvid:\s*([a-zA-Z0-9]+)/)?.[1];
                              return (<div key={idx} className="my-10 aspect-video w-full overflow-hidden rounded-2xl shadow-2xl bg-black ring-1 ring-white/10 text-center text-center text-center"><iframe src={`//player.bilibili.com/player.html?bvid=${bvid}&page=1&high_quality=1&danmaku=0&autoplay=0`} className="w-full h-full border-none" allowFullScreen loading="lazy" /></div>);
                            }
                            // 🌟 YouTube 识别逻辑
                            if (/\[\s*ytid:/.test(part)) {
                            ...
                            if (trimmedPart === '---') return <hr key={idx} className={`my-16 border-t ${isDarkMode ? 'border-white/10' : 'border-black/10'} text-center text-center` />;
                            return part.split('\n').map((line, lIdx) => {
                              if (line === '') return <div key={lIdx} className="h-8 text-center text-center" />;
                              return <p key={`${idx}-${lIdx}`} className="mb-5 min-h-[1.5em] text-center text-center text-center text-center" dangerouslySetInnerHTML={{ __html: applyInlineStyles(line) }} />;
                            });
                          });
                        })()}
                      </article>
                    )}
                    <div className={`flex flex-col sm:flex-row items-center justify-between gap-6 py-12 border-t border-dashed ${isDarkMode ? 'border-white/20' : 'border-black/10'} text-center text-center`}>
                       <p className={`text-sm font-black tracking-widest ${isDarkMode ? 'text-white/60' : 'text-black/40'} text-center text-center`}>如果喜欢这篇文章，请务必去支持一下原作者。</p>
                       <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className={`flex items-center gap-2 px-6 py-3 rounded-full text-[10px] font-black tracking-[0.2em] uppercase transition-all border ${isDarkMode ? 'border-white/20 hover:bg-white hover:text-black text-white' : 'border-black/10 hover:bg-black hover:text-white text-black'} text-center text-center`}>Top / 回到顶部 <ChevronUp size={14} /></button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            </main>
            <footer className="py-20 px-6 border-t border-black/5 dark:border-white/10 text-center opacity-40 text-[10px] tracking-widest font-serif uppercase text-black dark:text-white text-center text-center text-center">
              <div className="max-w-[600px] mx-auto space-y-3 normal-case leading-relaxed mb-12 text-center font-black text-center text-center text-center"><p>本站仅作为 Postype 平台 녘랜 (花汪) 同人文作品的翻译交流与存档使用，版权归原作者所有。</p><p>站内内容全是机翻，如有侵权请联系删除。</p><p className="font-bold text-center text-center">联系微博：<span>@恋花症-</span></p></div>
              <p onClick={(e) => { if (e.detail === 5) setIsAdmin(true); }} className="italic font-sans tracking-[0.2em] cursor-default select-none text-center text-center text-center text-center">© 2026 HW ARCHIVE.</p>
            </footer>
          </motion.div>
        )}
      </AnimatePresence>
