...
import { 
  ChevronLeft, ChevronUp, Moon, Sun, ShieldAlert, Heart, BookOpen, X, Sparkles 
} from 'lucide-react';
          ...
          <motion.div key="main-content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col min-h-screen text-[#333] dark:text-white">
            ...
            <AnimatePresence>
              {showUpdateNotice && !currentStory && (
                <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-6 bg-black/20 backdrop-blur-sm">
                  ...
                  >
                    {/* 米色主体 */}
                    <div className="bg-[#FDF5E6] dark:bg-[#2c2a26] border-2 border-amber-200 dark:border-amber-900/50 p-8 rounded-[2rem] shadow-2xl text-center">
                      <Sparkles className="mx-auto mb-4 text-amber-500" size={24} />
                      <h4 className="text-[11px] uppercase tracking-[0.3em] font-black text-amber-700 dark:text-amber-500 mb-6">Latest Archive / 最近更新</h4>
                      
                      <div className="space-y-4 mb-2">
                        {latestUpdates.map((story, i) => (
                          <div key={story.id} className="flex flex-col items-center">
                            <span className="text-sm font-black text-slate-800 dark:text-slate-100 tracking-tight leading-tight">
                              《{story.title}》
                            </span>
                            <span className="text-[9px] font-mono opacity-40 uppercase mt-1">
                              {story.date?.replace(/-/g, '.')} / {story.author}
                            </span>
                            {i < latestUpdates.length - 1 && <div className="w-1 h-1 bg-amber-200 rounded-full my-3" />}
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <button 
                      ...
                      className="mt-8 mx-auto flex items-center justify-center w-12 h-12 bg-slate-400 hover:bg-slate-500 text-white rounded-full shadow-lg transition-all active:scale-90"
                    >
                      <X size={24} />
                    </button>
                  </motion.div>
            ...
            <header className={`fixed top-0 left-0 right-0 z-50 px-6 py-4 flex justify-between items-center backdrop-blur-sm border-b ${isDarkMode ? 'bg-black/30 border-white/10' : 'bg-white/30 border-black/5'}`}>
              <div className="flex items-center gap-4">
                ...
                ) : ( <h1 className="text-sm uppercase tracking-widest font-sans font-semibold opacity-30 text-[#333] dark:text-white text-center">HW / ARCHIVE</h1> )}
              </div>
              <div className="flex items-center gap-3">
                ...
                {currentStory?.content && (
                  <div className="flex gap-1 ml-2 font-sans font-bold text-[#333] dark:text-white text-center">
            ...
            <main className="pt-24 pb-20 px-6 max-w-4xl mx-auto flex-grow w-full">
              <AnimatePresence mode="wait">
                {!currentStory ? (
                    ...
                    <section className="max-w-[700px] mx-auto">
                      {currentItems.map(s => (
                        <motion.button key={s.id} whileHover={{ x: 5 }} onClick={() => handleStoryClick(s)} className={`w-full grid grid-cols-[1fr_auto] py-8 border-b transition-colors text-left ${isDarkMode ? 'border-white/10 hover:border-white/30 text-white' : 'border-black/5 hover:border-black/20 text-[#333]'}`}>
                          <div className="flex items-baseline gap-3 text-center"><h3 className="text-xl font-medium mb-1">{s.title}</h3>{s.chapters && <BookOpen size={14} className="opacity-30" />}</div>
                          <div className="col-span-full flex gap-4 text-[10px] opacity-40 uppercase tracking-widest font-sans text-center"><span>{s.author}</span><span>{s.date?.replace(/-/g, '.')}</span>{s.chapters && <span>{s.chapters.length} 章节</span>}</div>
                        </motion.button>
                      ))}
                    </section>
                    {totalPages > 1 && (
                      <div className="flex justify-center items-center gap-12 mt-20 py-10 border-t border-dashed border-black/5 dark:border-white/5 text-center">
                  ...
                  <motion.div key="chapters" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[600px] mx-auto py-12 text-[#333] dark:text-white text-center">
                    ...
                    <div className="grid gap-4">
                      {currentStory.chapters?.map((chapter, idx) => (
                        <button key={idx} onClick={() => loadFullStory(currentStory, chapter.fileName, chapter.title)} className={`p-6 border rounded-xl text-left transition-all group flex justify-between items-center ${isDarkMode ? 'border-white/10 hover:bg-white/5' : 'border-black/5 hover:bg-black/5'} text-center`}>
                          <div className="text-center"><span className="text-[10px] opacity-30 block mb-1 font-sans font-bold text-center">CHAPTER {idx + 1}</span><span className="text-lg group-hover:pl-2 transition-all duration-300 text-center">{chapter.title}</span></div>
                          <div className="text-[10px] opacity-30 font-sans tracking-widest uppercase text-right text-center">{chapter.autoWordCount ? `${chapter.autoWordCount.toLocaleString()} 字` : '...'}</div>
                        </button>
                      ))}
                    </div>
                  </motion.div>
                ) : (
                  <motion.div key="content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-[700px] mx-auto text-[#333] dark:text-white text-center">
                    <header className="mb-16 border-b border-black/5 dark:border-white/5 pb-12 font-sans text-center text-center">
                      <h2 className="text-4xl font-serif font-light mb-8 leading-tight text-center">{currentStory.title}{currentStory.currentChapterTitle && (<span className="block text-xl opacity-50 mt-4 font-serif">— {currentStory.currentChapterTitle}</span>)}</h2>
                      <div className="text-[11px] uppercase tracking-[0.2em] opacity-40 space-y-1 font-bold text-center text-center"><p>作者: {currentStory.author}</p><p>时间: {currentStory.date?.replace(/-/g, '.')}</p><p>字数: {reading ? '...' : (currentStory.wordCount?.toLocaleString() || '...')}</p></div>
                      <a href={currentStory.sourceLink} target="_blank" rel="noopener noreferrer" className={`inline-block mt-8 text-[13px] font-bold tracking-[0.2em] underline underline-offset-8 decoration-1 transition-opacity ${isDarkMode ? 'text-[#90a4ae] hover:text-[#b0bec5]' : 'text-[#607d8b] hover:text-[#455a64]'} text-center`}>原链接 SOURCE →</a>
                    </header>
                    {reading ? (<div className="py-20 text-center opacity-20 tracking-widest text-xs uppercase animate-pulse text-center">Loading Content...</div>) : (
                        ...
                        {(() => {
                          ...
                          return parts.map((part, idx) => {
                            ...
                            if (/\[\s*quote\s*\]/.test(part)) {
                              ...
                              return (<blockquote key={idx} className="my-10 pl-5 border-l-4 border-slate-300 dark:border-slate-700 italic text-slate-500 dark:text-slate-400 bg-slate-100/30 dark:bg-white/5 py-6 rounded-r-xl text-center">{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}</blockquote>);
                            }
                            if (/\[\s*box\s*\]/.test(part)) {
                              const inner = part.replace(/\[\s*\/?box\s*\]/g, '').trim();
                              return (<div key={idx} className="my-10 p-8 bg-slate-100/60 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 rounded-2xl text-sm leading-relaxed shadow-sm ring-1 ring-black/5 dark:ring-white/5 text-center">{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-2 last:mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-4" />)}</div>);
                            }
                            if (/\[\s*bubble:/.test(part)) {
                              const isRight = part.includes(':R');
                              const inner = part.replace(/\[\s*bubble:[LR]\s*\]/g, '').replace(/\[\s*\/bubble\s*\]/g, '').trim();
                              return (<div key={idx} className={`flex ${isRight ? 'justify-end' : 'justify-start'} my-4 text-center`}><div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[14px] shadow-sm tracking-tight ${isRight ? 'bg-[#607d8b] text-white rounded-tr-none' : 'bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-slate-200 rounded-tl-none'} text-center`}>{inner.split('\n').map((l, i) => l.trim() ? <p key={i} className="mb-0" dangerouslySetInnerHTML={{ __html: applyInlineStyles(l) }} /> : <div key={i} className="h-2" />)}</div></div>);
                            }
                            if (/\[\s*bvid:/.test(part)) {
                            ...
                            if (trimmedPart === '---') return <hr key={idx} className="my-16 border-t border-black/10 dark:border-white/10 text-center" />;
                    ...
                    <div className={`flex flex-col sm:flex-row items-center justify-between gap-6 py-12 border-t border-dashed ${isDarkMode ? 'border-white/10' : 'border-black/10'} text-center`}>
                       <p className={`text-sm font-bold tracking-widest ${isDarkMode ? 'text-white/40' : 'text-black/40'} text-center`}>如果喜欢这篇文章，请务必去支持一下原作者。</p>
                       <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className={`flex items-center gap-2 px-6 py-3 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase transition-all border ${isDarkMode ? 'border-white/10 hover:bg-white hover:text-black' : 'border-black/10 hover:border-black hover:text-white'} text-center`}>Top / 回到顶部 <ChevronUp size={14} /></button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            </main>
            <footer className="py-20 px-6 border-t border-black/5 dark:border-white/10 text-center opacity-40 text-[10px] tracking-widest font-serif uppercase text-[#333] dark:text-white text-center">
