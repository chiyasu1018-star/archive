...
export default function Admin({ onBack }: { onBack: () => void }) {
  ...
  const fetchLogs = async () => {
    try {
      const res = await fetch(`${RAW_GITHUB_LOGS}?v=${Date.now()}`);
      const data = await res.json();
      setLogs(data || []);
    } catch (e) { setLogs([]); }
  };
  ...
  const handleSaveLogs = async () => {
    ...
    try {
      ...
      let sha = undefined;
      try {
        const { data: f } = await octokit.rest.repos.getContent({ owner: REPO_OWNER, repo: REPO_NAME, path: `public/stories/logs.json`, request: { cache: 'no-store' } });
        // @ts-ignore
        sha = f.sha;
      } catch (e) {}

      await octokit.rest.repos.createOrUpdateFileContents({
        owner: REPO_OWNER, repo: REPO_NAME, path: `public/stories/logs.json`,
        sha, message: `Update Logs`,
  ...
  const handleDeleteChapter = async (story: any, fileName: string, cTitle: string) => {
    ...
    try {
      const octokit = new Octokit({ auth: token });
      const { data: fileInfo } = await octokit.rest.repos.getContent({ owner: REPO_OWNER, repo: REPO_NAME, path: `public/stories/${fileName}`, request: { cache: 'no-store' } });
      await octokit.rest.repos.deleteFile({ owner: REPO_OWNER, repo: REPO_NAME, path: `public/stories/${fileName}`, message: `Delete`, // @ts-ignore
      ...
      await octokit.rest.repos.createOrUpdateFileContents({ owner: REPO_OWNER, repo: REPO_NAME, path: `public/stories/index.json`, // @ts-ignore
        sha: idxF.sha, message: `Update`, content: btoa(unescape(encodeURIComponent(JSON.stringify(indexData, null, 2)))), branch: BRANCH });
    ...
    <div className="min-h-screen p-6 max-w-4xl mx-auto font-sans text-sm text-slate-800 dark:text-slate-200">
      <header className="flex justify-between items-center mb-10 pb-4 border-b dark:border-white/10">
        ...
        <div className="flex gap-4 items-center">
            ...
            {/* 🌟 增加 Edit Logs 按钮 */}
            <button onClick={() => setView('logs')} className={`p-2 rounded-full transition-all ${view === 'logs' ? 'bg-blue-600 text-white' : 'hover:bg-black/5 dark:hover:bg-white/5 opacity-40'}`} title="Edit Logs"><Clock size={20}/></button>
        ...
        <div className="max-w-xl mx-auto space-y-8 animate-in fade-in">
           ...
           <div className="bg-white dark:bg-black/20 p-8 rounded-3xl border dark:border-white/10 space-y-6 shadow-2xl">
              <div className="space-y-1">
                 ...
                 <input value={logDate} onChange={e => setLogDate(e.target.value)} className="w-full bg-transparent border-b-2 dark:border-slate-800 py-2 text-xl font-bold outline-none" />
              </div>
              <div className="space-y-1">
                 <label className="text-[10px] font-black opacity-30 uppercase">What's New?</label>
                 <textarea value={logContent} onChange={e => setLogContent(e.target.value)} className="w-full h-32 bg-slate-50 dark:bg-white/5 p-4 rounded-xl outline-none" placeholder="例如：新增 R18 标识，优化 iPad 适配..." />
              </div>
            ...
            <div className="bg-white dark:bg-black/20 p-8 rounded-3xl border dark:border-white/10 space-y-6 shadow-2xl">
               ...
               <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <input value={chapterTitle} onChange={e => setChapterTitle(e.target.value)} className="w-full bg-blue-500/5 dark:bg-blue-500/10 p-3 rounded-xl outline-none border border-blue-500/20 text-blue-600 font-bold" placeholder="章节名" />
                  </div>
               </div>
               <div className="flex items-center gap-4 pt-2">
                 <label className="text-[10px] font-black opacity-30 uppercase tracking-widest">Rating / 分级</label>
                 <button type="button" onClick={() => setIsR18(!isR18)} className={`px-4 py-1.5 rounded-full text-[10px] font-black transition-all border ${isR18 ? 'bg-red-600 border-red-600 text-white shadow-lg shadow-red-600/20' : 'bg-transparent border-slate-300 dark:border-white/20 opacity-40 hover:opacity-100'}`}>{isR18 ? 'R18 RESTRICTED' : 'General'}</button>
               </div>
               <div className="space-y-1">
                  <label className="text-[10px] font-black opacity-30 uppercase tracking-widest">Source Link</label>
                  <input value={sourceLink} onChange={e => setSourceLink(e.target.value)} className="w-full bg-slate-100 dark:bg-white/5 p-3 rounded-xl outline-none text-xs" />
               </div>
               <div className="space-y-3">
                  ...
                  <textarea ref={textareaRef} value={content} onChange={e => setContent(e.target.value)} className="w-full h-[500px] bg-slate-50 dark:bg-white/5 p-6 rounded-2xl outline-none leading-relaxed text-base font-serif" placeholder="内容..." />
               </div>
